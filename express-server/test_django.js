...
async function runTests() {
    console.log('Starting Tests on Django Backend...');
    ...
    try {
        ...
        const execRes = await axios.post(`${BASE_URL}/execute`, {
            language: 'javascript',
            version: '18.15.0',
            code: 'console.log("Hello from Django!")'
        });
        console.log('Exec Status:', execRes.status);
        console.log('Exec Output:', execRes.data.stdout?.trim());
        if (execRes.data.stdout?.trim() === "Hello from Django!") {
